...
const handlerInsert = () => {
  ...
  axios.post('/api/support/testFileSave.do', formData).then((res) => {
...
const handlerUpdate = () => {
  ...
  axios.post('/api/support/testFileUpdate.do', formData).then((res) => {
    if (res.data.result === 'success') {
      alert('수정되었습니다.');
      modalState.$patch({ isOpen: false });
      emit('postSuccess');
...
const handlerDelete = () => {
  ...
  axios.post('/api/support/testFileDelete.do', formData).then((res) => {
    if (res.data.result === 'success') {
      alert('삭제되었습니다.');
      modalState.$patch({ isOpen: false });
      emit('postSuccess');
...
const searchDetail = () => {
  ...
  param.append('testId', id);

  axios.post('/api/support/testFileDetail.do', param).then((res) => {
...
const getFileImage = () => {
  ...
  axios.post('/api/support/testDownload.do', param, { responseType: 'blob' }).then((res) => {
...
<template>
  <Teleport to="body">
    <div class="test-modal-overlay">
      <form ref="formRef" class="test-modal-form modal-container">
        <label> 제목 :<input v-model="detail.testTitle" type="text" name="fileTitle" /> </label>
        <label> 내용 :<input v-model="detail.testContent" type="text" name="fileContent" /> </label>
        파일 :
        <input id="fileInput" type="file" name="file" @change="handlerFile" />
        <label class="test-img-label" htmlFor="fileInput"> 파일 첨부하기 </label>
        <div @click="downloadFile">
          <div>
            <label>미리보기</label>
            <img class="test-preview-image" :src="imageUrl" />
          </div>
        </div>
